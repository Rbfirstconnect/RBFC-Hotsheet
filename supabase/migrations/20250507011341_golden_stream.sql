/*
  ...
  1. Creates phone_pricing table if not exists
  2. Inserts initial pricing data
  3. Updates port-in, non-port, and upgrade prices
  4. Sets monthly payment and boost protect values
*/
...
SELECT 
  ...
  CASE 
    WHEN brand = 'Apple' AND name LIKE '%Pro%' THEN 15.00
...
-- Update port-in prices
UPDATE phone_pricing pp
SET port_in_price = (
  SELECT 
    CASE
      -- iPhones
      WHEN p.name = 'iPhone 11 64GB' THEN 0.00
      WHEN p.name = 'iPhone 12 64GB' THEN 0.00
      WHEN p.name = 'iPhone 13 128GB' THEN 49.99
      WHEN p.name = 'iPhone 15 128GB' THEN 149.99
      WHEN p.name = 'iPhone 15 Pro 256GB' THEN 619.99
      WHEN p.name = 'iPhone 16 128GB' THEN 449.99
      WHEN p.name = 'iPhone 16 Pro Max 256GB' THEN 999.99
      WHEN p.name = 'iPhone 16e 128GB' THEN 199.99
      -- Samsung
      WHEN p.name LIKE 'Samsung A%' THEN 0.00
      WHEN p.name = 'Samsung Galaxy S24 256GB' THEN 449.99
      WHEN p.name = 'Samsung Galaxy S24 Ultra 256GB' THEN 699.99
      WHEN p.name = 'Galaxy S24+ 256GB' THEN 399.99
      -- Motorola
      WHEN p.name LIKE 'Moto G%' THEN 0.00
      WHEN p.name = 'Motorola Edge 2023' THEN 49.99
      WHEN p.name = 'Motorola Edge+ 2023' THEN 269.99
      WHEN p.name = 'Motorola Razr 2024' THEN 49.99
      -- Others
      WHEN p.brand IN ('Celero', 'Summit') THEN 0.00
      ELSE 99.99
    END
  FROM phones p
...
-- Update non-port prices
UPDATE phone_pricing pp
SET non_port_price = (
  SELECT 
    CASE
      -- iPhones
      WHEN p.name = 'iPhone 11 64GB' THEN 49.99
      WHEN p.name = 'iPhone 12 64GB' THEN 49.99
      WHEN p.name = 'iPhone 13 128GB' THEN 149.99
      WHEN p.name = 'iPhone 15 128GB' THEN 249.99
      WHEN p.name = 'iPhone 15 Pro 256GB' THEN 719.99
      WHEN p.name = 'iPhone 16 128GB' THEN 629.99
      WHEN p.name = 'iPhone 16 Pro Max 256GB' THEN 999.99
      WHEN p.name = 'iPhone 16e 128GB' THEN 299.99
      -- Samsung
      WHEN p.name = 'Samsung A03s' THEN 15.00
      WHEN p.name = 'Samsung A15 5G 64GB' THEN 0.00
      WHEN p.name = 'Samsung A15 5G 128GB' THEN 15.00
      WHEN p.name = 'Samsung A16' THEN 25.00
      WHEN p.name = 'Samsung A25' THEN 79.99
      WHEN p.name = 'Samsung A35' THEN 149.99
      WHEN p.name = 'Samsung Galaxy S24 256GB' THEN 859.99
      WHEN p.name = 'Samsung Galaxy S24 Ultra 256GB' THEN 999.99
      WHEN p.name = 'Galaxy S24+ 256GB' THEN 399.99
      -- Motorola
      WHEN p.name = 'Motorola Edge 2023' THEN 79.99
      WHEN p.name = 'Motorola Edge+ 2023' THEN 269.99
      WHEN p.name = 'Moto G Stylus 5G 2024' THEN 49.99
      WHEN p.name = 'Moto G 5G 2024' THEN 0.99
      WHEN p.name = 'Motorola Razr 2024' THEN 149.99
      -- Others
      WHEN p.name = 'Celero 5G 2024' THEN 29.99
      WHEN p.name = 'Celero 5G+ 2024' THEN 99.99
      WHEN p.name = 'Celero 5G SC' THEN 19.99
      WHEN p.name = 'Summit 5G' THEN 10.00
      WHEN p.name = 'Summit Flip' THEN 29.99
      ELSE 199.99
    END
  FROM phones p
  WHERE p.id = pp.phone_id
...
-- Update upgrade prices to match non-port prices
UPDATE phone_pricing pp
SET upgrade_price = non_port_price + 50;
