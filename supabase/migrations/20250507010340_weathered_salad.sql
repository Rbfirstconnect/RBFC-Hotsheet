/*
  ...
  1. Changes
    - Updates device financing prices for all phones
    - Ensures correct monthly payments and boost protect fees
    - Maintains existing plan associations
*/

-- Update iPhone financing
...
SET
  ...
  END,
  boost_protect = CASE
    WHEN phone_id IN (SELECT id FROM phones WHERE name LIKE '%S24%' OR name LIKE '%S25%' OR name LIKE '%Ultra%' OR name LIKE '%Flip%') THEN 12.00
    ELSE 8.00
  END
WHERE phone_id IN (SELECT id FROM phones WHERE brand = 'Samsung');

-- Update Motorola financing
UPDATE phone_pricing
SET
  monthly_price = CASE
